...
@interface ESCYUV420MetalVideoPlayViewController ()

@property(nonatomic,strong)ESCAudioStreamPlayer* audioPlayer;

@property(nonatomic,weak)ESCMetalView* metal_view;
...
@implementation ESCYUV420MetalVideoPlayViewController
...
//    });
    ...
    ESCMetalView *metal_view = [[ESCMetalView alloc] init];
    self.metal_view = metal_view;
    [self.view addSubview:metal_view];
    self.metal_view.type = ESCVideoDataTypeYUV420;
    self.metal_view.showType = ESCOpenGLESViewShowTypeAspectFit;
    [self.metal_view mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (NSData *)copyFrameDataFromSource:(UInt8 *)src
...
//        NSInteger pts = videoFrame->pts;
        ...
        if (self.metal_view) {
            NSData *ydata = [self copyFrameDataFromSource:videoFrame->data[0] linesize:videoFrame->linesize[0] width:videoFrame->width height:videoFrame->height];

            NSData *udata = [self copyFrameDataFromSource:videoFrame->data[1] linesize:videoFrame->linesize[1] width:videoFrame->width / 2 height:videoFrame->height / 2];

            NSData *vdata = [self copyFrameDataFromSource:videoFrame->data[2] linesize:videoFrame->linesize[2] width:videoFrame->width / 2 height:videoFrame->height / 2];
...
//            [self.openGLESView loadYUV420PDataWithYData:ydata uData:udata vData:vdata width:self.width height:self.height];
            [self.metal_view loadYUV420PDataWithYData:ydata uData:udata vData:vdata width:self.width height:self.height];
            //                    }
